...
auto change_last_bits(unsigned char &RGB,
                      std::vector<std::bitset<8>> const &bits, int &index_set,
                      int &index_bit) -> bool {
...
/// Converts the string given to its bit by bit representation closed into the
/// vector if bitsets.
///
///@param str
///@param tail If set to true, function adds the "mark" at the end that
///signifies the end.
///@returns True if there are more bits that need to be changed.
auto conv_str_to_bin(std::string const &str, bool const tail)
...
/// Clumsy way of converting bits represented in a vector to a message
/// represeneted in a vector.
///
///@param bits
...
///@returns True if there are more bits that need to be changed.
auto bad_change_last_bits(int &RGB, std::vector<int> const &bin, int &index)
    -> bool {
  int LSB = RGB % 2;
  int SLSB = (RGB / 2) % 2;
...
auto bad_conv_str_to_bin(std::string const &str, bool const tail)
    -> std::vector<int> {
  std::vector<int> bin;
