...
namespace flags{
	...
	auto help() -> void{
		std::cout << "\n~~~~~~~~~~\nSupported extensions: bmp, ppm (P3 type)\nAvailable options: -i/--info, -e/--encrypt, -d/--decrypt, -c/--check, -h/--help\n\n";
		std::cout << "-i <path> - checks if a file specified in the <path> can be modified.\n\n";
		std::cout << "-e <path> <message> - opens, if possible, <path> and creates mirror file with the <message> in it.\n"
		...
		std::cout << "-c <path> <message> - checks if <message> can be written to a file specified in the <path>.\n\n";
		std::cout << "Formula e.g. : stego.exe -i \"there/here/picture.ppm\" -e here/there/image.ppm \"Behind you\"\n";
		std::cout << "Notes: <path> cannot be longer than 70 characters. Max 12 arguments.\n" 
					 "      \"-e\" produces a new file, so using -e and -d on the same file at once is poinless and will produce Gibberish.\n"
					 "		Only one encryption and/or decryption per execute (i.e. cant use -e twice).\n"
					 "		To encrypt ppm P6 files you can use online converter and turn them into P3s\n~~~~~~~~~~\n";
	}
	...
	auto check_extension(std::string const& path) -> bool{
		if (path.size() > 70 || path.find(".") == std::string::npos)		{std::cout << "Incorrect path\n"; help(); return false;}
		...
		if (std::filesystem::exists(path)){
			...
			print_last_write_time(std::filesystem::last_write_time(path));
			std::cout << "Permissions: ";
			if (print_permission(std::filesystem::status(path).permissions()) == '-'){
				std::cout << "WARNING!!!: no premission to read this file. Encryption and decryption will probably fail\n";
			}
			std::cout << "\n";
	...
	///@param inner_arg_counter Used to check if their is nothing in bweetwen flags and if some of them were not used twice.
	///@returns False if something went wrong.
	auto usual_check(std::vector<std::string>::iterator &curr, std::vector<std::string>::iterator const& end, std::vector<std::string> const& args, int const& next_flag_in, int& inner_arg_counter) -> bool{
		inner_arg_counter++;
		if (curr + (next_flag_in - 1) >= end)	{std::cout << "Missing argument\n"; help(); return false;}
		if (!check_extension(*(curr + 1)))	{std::cout << "opop" << "\n";return false;}
	...
	auto parse(int const argc, char *arqv[]) -> bool{
		if (argc <= 2 || argc > 12)		{help(); return false;}
		...
		for (auto curr = args.begin(), end = args.end(); curr != end ; ++curr, arg_counter++){
			if (*curr == "-h" || *curr == "--help")		{help(); return false;}
			if (*curr == "-i" || *curr == "--info"){
				if(!usual_check(curr, end, args, 2, inner_arg_counter))	{return false;}
			}
			if ((*curr == "-e" || *curr == "--encrypt") && params::path_to_write.size() == 0){
				if(!usual_check(curr, end, args, 3, inner_arg_counter))	{return false;}
				params::path_to_write = *(curr - 1);
				params::mess = *(curr);
			}
			if ((*curr == "-d" || *curr == "--decrypt") && params::path_to_read.size() == 0){
				if(!usual_check(curr, end, args, 2, inner_arg_counter))	{return false;}
				params::path_to_read = *(curr);
			}
			if (*curr == "-c" || *curr == "--check"){
				if(!usual_check(curr, end, args, 3, inner_arg_counter))	{return false;}
				if (((std::filesystem::file_size(*(curr - 1))) - ((get_extension(*(curr - 1)) == "mpp.")?4:138)) < (((*(curr)).size() * 8))){
					std::cout << "There is no place to write this message\n";
				}else std::cout << "Message can be written to that file\n";
			}
			if ((arg_counter + 1) != inner_arg_counter)	{std::cout << "Wrong arguments\n"; help(); return false;}	
		}
		return true;
	}
